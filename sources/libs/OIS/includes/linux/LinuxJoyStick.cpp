...
//6 		 D PadX 4
//7 		 D PadY 4
	char AxisMask[8] = {0,0,2,1,1,3,4,4};
	char XYZMask[8] = {0,1,2,0,1,2,0,1};
	...
	for(int i = 0; i < OIS_MAX_DEVICES; ++i )
	{
		...
		if( joyStick != -1 )
		{
			...
			if( ioctl(joyStick, JSIOCGNAME(OIS_DEVICE_NAME), nameBuff) < 0 )
				...
				js.vendor = nameBuff;
/* Quick hack at testing ForceFeed Back...
ff_effect test;
test.type = FF_CONSTANT;
test.id = -1; //Create new effect
test.direction = 0;
test.trigger.button = 0;
test.trigger.interval = 1000; //ms
test.replay.length = 500; //ms
test.replay.delay = 200; //ms

test.u.constant.level = 2000; //neg values ok
test.u.constant.envelope.attack_length = 3000;	// Duration of attack (ms)
test.u.constant.envelope.attack_level = 1000;	// Level at beginning of attack
test.u.constant.envelope.fade_length = 2000;	// Duration of fade (ms)
test.u.constant.envelope.fade_level = 500;	// Level at end of fade

			if( ioctl(joyStick, EVIOCSFF, &test) < 0 )
