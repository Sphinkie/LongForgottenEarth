	...
	mMouse(mouse), mKey(key), mJoy(joy), mSceneMgr(scnMgr)
{
	...
	oldJoyListener = mJoy->getEventCallback();
...
bool ActionConfig::buttonPressed(const OIS::JoyStickEvent &arg, int button)
{
	...
	//Set back old listener
	mJoy->setEventCallback(oldJoyListener);
		...
		case A_MOVE:
		{
			...
			else if( arg.component->cType == OIS::OIS_Axis )
			{
				...
				else
				{
					...
					//Up/Down
					mTranslateVector.y = mMoveScale * axis->abZ/32767.0f;
					//Move left/right
					mTranslateVector.x = mMoveScale * axis->abX/32767.0f;
					//Move forward/back
					mTranslateVector.z = -mMoveScale * axis->abY/32767.0f;
				}
			}
			else if( arg.component->cType == OIS::OIS_POV )
			{
				const OIS::Pov* pov = static_cast<const OIS::Pov*>(arg.component);
				//xxx todo.. use all values
				if( pov->value == 0 ) //Going up
					mTranslateVector.z = -mMoveScale;
				//else if( pov->value == 4500 ) //Going right/up
				else if( pov->value == 9000 ) //Going right
					mTranslateVector.x = mMoveScale;
				//else if( pov->value == 13500 ) //Going down/right
				else if( pov->value == 18000 ) //Going down
					mTranslateVector.z = mMoveScale;
				//else if( pov->value == 22500 ) //Going down/left
				else if( pov->value == 27000 ) //Going left
					mTranslateVector.x = -mMoveScale;
				//else if( pov->value == 32500 ) //Going left/up
				else if( pov->value == -1 ) //stopped
	...
	switch( arg.ActionID )
	{
		...
		case A_LOOK:
		{
			...
			else if( arg.component->cType == OIS::OIS_Axis )
			{
				...
				{
					//absolute only axis
					mRotXScale = -axis->abX/32767.0f * 0.05;
					mRotYScale = -axis->abY/32767.0f * 0.01;
				}
			}
			break;
		}
...
void ActionConfig::handleDone( const OIS::ActionMapArg& arg )
{
	...
	if( static_cast<const OIS::Button*>(arg.component)->pushed )
	{
		...
		CEGUI::WindowManager::getSingleton().getWindow(
			(CEGUI::utf8*)"Main")->setVisible(true);
