...
public:
	...
	GuiFrameListener(RenderWindow* win, Camera* cam, CEGUI::Renderer* renderer)
        	: ExampleFrameListener( win, cam, true, true ), mGUIRenderer(renderer), mShutdownRequested(false), mJoy(0)
	{
	...
	OIS::JoyStick* getJoyStick() 
	{
		...
		if( OIS::InputManager::getSingletonPtr()->numJoysticks() > 0 )
			mJoy = (OIS::JoyStick*)(OIS::InputManager::getSingletonPtr()->createInputObject(OIS::OISJoyStick, true));
	...
	bool keyPressed( const OIS::KeyEvent &arg )
	{
		...
		CEGUI::System::getSingleton().injectChar( ((OIS::Keyboard*)arg.device)->lookupCharMapping( arg.key ) );
		return true;
	}
