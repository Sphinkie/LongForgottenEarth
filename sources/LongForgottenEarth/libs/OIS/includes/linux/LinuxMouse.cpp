...
void LinuxMouse::capture()
{
	...
	if( moved == true )
	{
		mState.relX = mState.abX - oldX;
		mState.relY = mState.abY - oldY;
		...
		{
			//Ok, check if we are near the edge, so we can bring back to
			//center, as there are no events when mouse moves past grabbed edge
			if( mState.abX < 5 || mState.abX > mState.width - 5 ||
			    mState.abY < 5 || mState.abY > mState.height - 5 )
			{
				oldX = mState.abX = mState.width >> 1;
				oldY = mState.abY = mState.height >> 1;
				XWarpPointer(display, None, window, 0, 0, 0, 0,
					     mState.abX, mState.abY);
			}
		}
	}
...
bool LinuxMouse::_injectXEvent(XEvent &event)
{
	...
	{
		mState.abX = event.xmotion.x;
		mState.abY = event.xmotion.y;
