...
inline noise::uint8 BlendChannel (const uint8 channel0, const uint8 channel1,
  float alpha)
{
...
// Unpacks a floating-point value into four bytes.  This function is specific
// to Intel machines.  A portable version will come soon (I hope.)
...
NoiseImage::NoiseImage (int width, int height)
{
  InitObj ();
  SetSize (width, height);
}

NoiseImage::NoiseImage (const NoiseImage& rhs)
{
  InitObj ();
  CopyImage (rhs);
}
...
NoiseImage& NoiseImage::operator= (const NoiseImage& rhs)
{
  CopyImage (rhs);
...
void NoiseImage::Clear (const Color& value)
{
  if (m_pImage != NULL) {
    for (int y = 0; y < m_height; y++) {
...
void NoiseImage::CopyImage (const NoiseImage& source)
{
...
void NoiseImage::DeleteImageAndReset ()
{
  delete[] m_pImage;
  InitObj ();
}

Color NoiseImage::GetValue (int x, int y) const
{
  if (m_pImage != NULL) {
    if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
      return *(GetConstSlabPtr (x, y));
...
void NoiseImage::SetSize (int width, int height)
{
  if (width < 0 || height < 0
    || width > RASTER_MAX_WIDTH || height > RASTER_MAX_HEIGHT) {
    // Invalid width or height.
...
void NoiseImage::SetValue (int x, int y, const Color& value)
{
  if (m_pImage != NULL) {
    if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
      *(GetSlabPtr (x, y)) = value;
...
void NoiseImage::TakeOwnership (NoiseImage& source)
{
...
//	DEBUG_LOG("   GetValue ("+ITOA((float)xCur)+", "+ITOA((float)zCur)+")");
        finalValue = planeModel.GetValue (xCur, zCur);
//	DEBUG_LOG(ITOA(finalValue));
      } else {
...
void NoiseMapBuilderSphere::Build ()
{
  ...
  for (int y = 0; y < m_destHeight; y++) {
    ...
    for (int x = 0; x < m_destWidth; x++) {
//	DEBUG_LOG("   GetValue ("+ITOA((float)curLat)+", "+ITOA((float)curLon)+")");
      float curValue = (float)sphereModel.GetValue (curLat, curLon);
//	DEBUG_LOG(ITOA(curValue));
      *pDest++ = curValue;
      curLon += xDelta;
    }
    curLat += yDelta;
...
void RendererNormalMap::Render ()
{
  if ( m_pSourceNoiseMap == NULL
    ...
    || m_pSourceNoiseMap->GetWidth  () <= 0
    || m_pSourceNoiseMap->GetHeight () <= 0) {
    TRACE("*** ERROR : RendererNormalMap::Render() : Invalid Parameters");
    throw noise::ExceptionInvalidParam ();
  }
