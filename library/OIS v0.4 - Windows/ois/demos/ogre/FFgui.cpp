...
protected:
	...
	void createScene(void)
	{
		...
		//--------------- setup GUI system ---------------------//
		mGUIRenderer = new OgreCEGUIRenderer(mWindow, RENDER_QUEUE_OVERLAY, false, 3000);
	...
	void createFrameListener(void)
	{
		...
		if( !ff )
			OIS_EXCEPT(E_NotSupported, "No FF capable device found!");
	...
	bool handleCreate(const CEGUI::EventArgs& e)
	{
		...
		int index = list->getItemIndex( list->getFirstSelectedItem() );
